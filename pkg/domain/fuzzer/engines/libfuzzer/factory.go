...
// Factory implements FuzzerFactory for LibFuzzer
type Factory struct {
	log logrus.FieldLogger
}

// NewFactory creates a new LibFuzzer factory
func NewFactory(log logrus.FieldLogger) types.FuzzerFactory {
	...
	return &Factory{
		log: log.WithField("factory", "libfuzzer"),
	}
}

// CreateFuzzer creates a new LibFuzzer instance
...
func (f *Factory) GetSupportedTypes() []string {
	return []string{types.FuzzerTypeLibFuzzer.String()}
}

// IsSupported checks if a fuzzer type is supported
func (f *Factory) IsSupported(fuzzerType string) bool {
	return fuzzerType == types.FuzzerTypeLibFuzzer.String() || fuzzerType == "libfuzzer"
}
