...
// NewMonitoringService creates a new monitoring service using repository interfaces.
...
func (s *monitoringService) UpdateSystemMetrics(ctx context.Context) error {
	// Update bot metrics
	if err := s.updateBotMetrics(ctx); err != nil {
		return err
	}

	// Update job metrics
	if err := s.updateJobMetrics(ctx); err != nil {
		return err
	}
...
func (s *monitoringService) updateBotMetrics(ctx context.Context) error {
...
func (s *monitoringService) updateJobMetrics(ctx context.Context) error {
	jobs, err := s.jobRepo.ListAll(ctx)
	if err != nil {
		return err
	}
